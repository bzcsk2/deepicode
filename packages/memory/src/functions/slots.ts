import type { ISdk } from "../runtime/index.js";
  ...
  sdk.registerFunction(
    ...
    async (data: { label?: string; content?: string }) => {
      ...
      return withKeyedLock(`slot:${label}`, async () => {
        ...
        if (!slot) return { success: false, error: "slot not found (use mem::slot-create first)" };
        if (slot.readOnly) return { success: false, error: "slot is read-only" };
        if (safeContent.length > slot.sizeLimit) {
          return {
            success: false,
            error: `content exceeds sizeLimit (${safeContent.length} > ${slot.sizeLimit})`,
            sizeLimit: slot.sizeLimit,
          };
        }
        const updated: MemorySlot = { ...slot, content: safeContent, updatedAt: nowIso() };
        await kv.set(scopeKv(scope), label, updated);
        ...
        return { success: true, slot: updated, size: safeContent.length };
      });
    },
  );
