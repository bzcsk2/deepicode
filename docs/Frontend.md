...
根据对 `deepreef` 项目 `packages/tui/src` 目录下源代码的完整梳理，控制 **TUI 显示风格**（颜色、布局、视觉组件）和 **字样**（文本内容、提示语）的核心文件可以归为以下几类：
...
| 文件 | 作用 |
|------|------|
| **`packages/tui/src/FullscreenLayout.tsx`** | 全屏/非全屏布局切换、终端头部（`TerminalHeader`）的标题样式（`deepreef agent terminal`）。 |
