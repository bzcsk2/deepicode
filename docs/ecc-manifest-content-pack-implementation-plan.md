# Deepreef Claude Code Plugin 协议兼容与 ECC 实施方案

> 供实施 Agent 使用。本文定义 Deepreef plugin 系统如何原生兼容 Claude Code Plugin 协议，并以 `/vol4/Agent/ECC` 作为首个完整兼容目标。本文是实施方案，不代表功能已经完成。

## 0.1 架构校正：以 Claude Code Plugin 协议为主标准

对照 `/vol4/Agent/best-claude-code` 源码后，确认 Claude Code plugin 本身就是 manifest 驱动的资源包插件系统，不是 Deepreef 当前的可执行 JS runtime plugin。
...
**Deepreef 对 Claude Code Plugin 协议的兼容实现。**

不要先发明一套与 Claude Code 不同的 Deepreef 内容包协议。Deepreef 专属 manifest 只能作为未来扩展，不能成为 ECC 接入的前置条件。

ECC 的标准 Claude plugin surface 可以由 Deepreef 按 Claude Code 规则直接发现。ECC 的以下能力属于 ECC 自身扩展层，不属于 Claude Code plugin 基础协议：
...
3. **遗留层：Deepreef Executable Module Plugin**
...
**改造 Deepreef 的 plugin 架构，让它原生兼容 Claude Code Plugin 协议；ECC 通过该协议直接接入。**

不要把 ECC 直接内置进 Deepreef 核心，也不要为 ECC 写一次性 wrapper 后结束。ECC 应作为兼容性样本和验收对象，而 Deepreef 获得的是通用内容包能力。

正确边界：

- Deepreef 核心继续负责 Engine、工具执行、安全 hook、MCP host、TUI。
- `@deepreef/plugin` 负责识别、解析、校验和汇总 Claude Code plugin manifest。
- ECC 原始仓库作为外部内容包被引用或安装，不复制整个仓库进 Deepreef。
...
2. **新增 Claude Code plugin 类型，而不是替换现有插件类型。**
   现有插件是 Deepreef 自定义可执行模块；ECC 使用的是 Claude Code manifest 插件协议。

3. **优先引用 ECC 源文件，避免复制 ECC 内容。**
   Skills、Agents、Rules 等应通过路径注册和按需读取实现；只有必要的适配逻辑写在 Deepreef。
...
7. **保持包边界清晰，避免循环依赖。**
   `@deepreef/plugin` 可以产出内容包解析结果；`@deepreef/tools`、`@deepreef/core`、`@deepreef/mcp` 分别消费对应资产。

## 2. 已确认现状

### 2.1 Deepreef 当前 plugin 契约

当前 `@deepreef/plugin` 要求插件模块导出：
...
| 当前名称 | 建议名称 | 含义 |
|---|---|---|
| runtime plugin | executable module plugin / legacy runtime plugin | Deepreef 自定义 JS/TS 模块，导出 `id` 和 `server()` |
| content-pack plugin | Claude Code plugin / manifest plugin | `.claude-plugin/plugin.json` + commands/agents/skills/hooks/MCP 资源包 |

两者的差异：

| 维度 | Deepreef 旧 runtime plugin | Claude Code plugin |
|---|---|---|
...
Deepreef 兼容时应优先复刻这些协议语义。

### 2.3 Deepreef 当前 Skills
...
- 读取 `.deepreef/mcp.json`。
- 只支持 stdio MCP：`command`、`args`、`env`。
- 不支持 `type: "http"` / `url` transport。
- auth store 会注入 `MCP_API_KEY` 和 `DEEPREEF_MCP_API_KEY`。
...
- `.opencode/plugins/ecc-hooks.ts` 是 OpenCode 原生插件，依赖 `@opencode-ai/plugin`，不能直接用于 Deepreef。

## 3. 目标定义

首版目标是让 Deepreef 支持以下配置：
...
1. 识别目录插件。
2. 自动发现 `.deepreef-plugin.json`、`.codex-plugin/plugin.json`、`.claude-plugin/plugin.json` 等 manifest。
3. 解析 ECC profile/module/component。
4. 将选中的 skills 注册到 `Skill` 工具。
5. 将选中的 agents 注册到 Deepreef agent registry。
...
export interface ContentPackManifest {
  ...
  sourceKind: "deepreef" | "codex" | "claude" | "ecc"
  skills: string[]
  agents: string[]
  rules: string[]
  commands: string[]
...
- `@deepreef/plugin` 只负责解析和 resolve 内容资产。
- 不要让 `@deepreef/plugin` 直接 import `@deepreef/tools`、`@deepreef/mcp` 或 TUI。
...
2. `<root>/.deepreef-plugin.json`
3. `<root>/deepreef-plugin.json`
...
- `target: "deepreef"` 表示最终消费方是 Deepreef。
- 发现 manifest 时，Deepreef 可优先使用 Codex manifest 作为最接近的基础入口。
- 不能因为 ECC 尚未声明 `deepreef` target，就把所有 module 过滤掉。
...
filterByTarget(selected, options.target ?? "deepreef", options.targetMode ?? "compatible")
...
- `compatible`：默认。`deepreef` 不在 targets 中时，根据 module kind 和 Deepreef 已实现的消费能力决定是否保留，并产生兼容诊断。
- `ignore`：忽略 targets，只按 profile/module/component 选择；仍受各资产开关和安全策略控制。

Deepreef `compatible` 模式建议：

| Module kind | Deepreef 已有消费能力 | 处理 |
|---|---|---|
| `skills` | 是 | 保留 |
...
- `hooks-runtime` 在 ECC profiles 中可能默认包含，但 Deepreef 首版 `hooks.enabled` 默认 false，因此即使 module 被选中，也只记录为 disabled asset。
- `platform-configs` 中包含 `.codex-plugin`、`.opencode`、`mcp-configs` 等目录，不能盲目导入全部平台配置。
- `commands-core` 可被选中，但 Deepreef 首版不直接注册 slash commands。
...
实施 agent 必须先确认 Deepreef 实际工具名，不能只凭表格写死。
...
| 具体 provider model | 若 Deepreef 配置支持则使用，否则 inherit |
...
ECC 自己也说明 Skills 是更 canonical 的能力，commands 是 legacy shims。Deepreef 首版不要把 ECC commands 直接加入 slash command 解析。
...
- 解析 `hooks/hooks.json`。
- 按 `matcher` 匹配 Deepreef tool name。
...
当前 `McpHost.loadConfig()` 只从 `.deepreef/mcp.json` 读取。建议新增：
...
7. 创建 Engine 时传入：
   ...
   - plugin hook manager
8. 创建 MCP host 并加载 `.deepreef/mcp.json` 和允许的内容包 MCP。
...
当前 Zod 适配已经在 `@deepreef/plugin` 包内实现：
...
但这些能力尚未进入 Deepreef CLI/Engine 的生产路径。当前 `packages/cli/src/tui.ts` 只读取 `.deepreef/plugins.json` 计算展示数量，没有创建 `PluginRuntime`，也没有把 executable plugin tools 注册进 Engine。

Claude Code Plugin/ECC 接入必须同时完成这条生产链路：

```text
.deepreef/plugins.json
...
7. Claude Code manifest plugin 与旧 executable plugin 必须能够在同一 `.deepreef/plugins.json` 中同时加载。
...
不要让 `@deepreef/core` 直接依赖 `@deepreef/plugin`。由 CLI 负责把 plugin tools 转换并注册到 Engine。

## 15. 包依赖建议

当前依赖：

- `@deepreef/plugin` 依赖 `@deepreef/core`
- `@deepreef/tools` 依赖 `@deepreef/core`
- `@deepreef/mcp` 依赖 `@deepreef/core` 和 `@deepreef/tools`

避免让 `@deepreef/plugin` 依赖 `@deepreef/tools` 或 `@deepreef/mcp`。

推荐：

- `@deepreef/plugin` 产出 plain data。
- `@deepreef/tools` 可依赖 `@deepreef/plugin` 的类型吗？尽量不要，避免包交叉。可以复制极小 `SkillSource` 类型，或把共享类型放到 `@deepreef/core`。
- 更干净方案：在 `@deepreef/core` 新增内容资产通用类型，例如 `ContentAsset`、`AgentDefinition` 扩展。
...
- timeout 生效。
- matcher 能从 `PreToolUse` 匹配到 Deepreef tool。
...
- 没有 `.deepreef/plugins.json` 时行为保持现状。
...
1. Deepreef 支持目录型 manifest 内容包插件。
2. ECC 可通过 `.deepreef/plugins.json` 以 `/vol4/Agent/ECC` 路径加载。
3. 不复制 ECC 整仓内容进 Deepreef。
...
- PreToolUse/PostToolUse 到 Deepreef hook 的桥接。
