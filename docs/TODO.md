...
| Session | `.deepreef/sessions/*.jsonl`，best-effort append | 不要让持久化失败阻塞主流程 |
...
| 2 | `IC-20` Runtime checkpoint v2 | 为 Deepreef 增加任务运行态快照，支撑压缩、恢复和长任务诊断。 |
...
- 在 Deepreef 现有 `.deepreef/sessions/*.jsonl` 之外，增加 additive checkpoint。
...
```text
.deepreef/sessions/<sessionId>.checkpoint.json
```
...
- 在 Deepreef 现有 `RuntimeLogger` 上增加可统计事件 schema。
...
2. 所有事件默认受 `DEEPREEF_LOG_LEVEL` 控制。
...
- 评估 Deepreef 是否需要项目事实/用户偏好/会话摘要三类 Markdown 记忆。
