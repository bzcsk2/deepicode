...
- 参考 `/vol4/Agent/best-claude-code` 的 AgentTool 设计，但不要照搬所有后台 daemon、remote agent、team swarm 能力。Deepreef 第一阶段先实现本地临时子代理。
...
- `subagent_type` 省略，并且 `FEATURE_FORK_SUBAGENT=1` 或 deepreef 配置开启时，走 fork。
...
Deepreef 第一版 fork 可以先做同步，不强制后台：
...
- 策略持久化文件是 `.deepreef/context.json`，不要合并进主配置。
...
4. 人工验收：
   ...
   - 退出并重启，确认 `.deepreef/context.json` 策略仍然生效。
