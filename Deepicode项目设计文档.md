...
### 6.1 TUI 渲染引擎：Ink/React

TUI 层基于从 best-claude-code 复制的 `@anthropic/ink` 框架（forked React 终端渲染器）。该框架的核心能力：

- **React 19 + Yoga 布局引擎**：Flexbox 布局模型，支持 `flexDirection`、`flexGrow`、`flexShrink`、`position: absolute` 等 CSS 属性
- **ScrollBox 虚拟滚动**：利用 DECSTBM（终端滚动区域）实现视口裁剪，只渲染可见行
- **Alternate Screen**：ANSI `\x1b[?1049h` 全屏模式，消除滚动闪烁
- **鼠标交互**：SGR mouse tracking（点击、拖拽选择、滚轮滚动）
- **50+ 公共 API**：`Box`、`Text`、`ScrollBox`、`AlternateScreen` 组件；`useInput`、`useTerminalSize`、`useKeybinding` 等 hooks；`Dialog`、`Pane`、`ProgressBar` 等主题组件

框架代码位于 `packages/ink/`，完全自包含（146 文件 / ~27K 行），不依赖 deepicode 的任何业务代码。deepicode 专属的业务组件（消息渲染、输入框、状态栏等）位于 `packages/tui/src/`，基于 `@deepicode/ink` 的公共 API 编写。

**架构图**：
```
packages/ink/src/          ← 复制自 best-claude-code，不改
packages/tui/src/
  ├── App.tsx              ← deepicode 顶层组件
  ├── FullscreenLayout.tsx ← 复制自 best-claude-code，~10 个 import 已适配
  ├── DeepiMessages.tsx    ← 新写，渲染 deepicode 的消息格式
  ├── DeepiPromptInput.tsx ← 新写，输入框
  ├── bridge.tsx           ← 新写，Engine → React state
  └── ...                  ← 其他新写组件
```

### 6.2 集中式状态管理 (`state.ts`)

采用 `AgentState` 声明式更新，不直接修改对象树。`processEvents()` 函数接收事件队列，派生出全新的 TUI 渲染树，支持状态回溯。

在 Ink/React 架构中，状态桥接由 `bridge.tsx` 负责——遍历 `engine.submit()` 的 AsyncGenerator，通过 `useState` + `setState(prev => ({...}))` 将引擎事件转为 React 组件状态，触发 Ink 自动重渲染。
...
- **TUI 适配**：拉模式 AsyncGenerator 通过 `bridge.tsx` 转为推模式的 React `setState` 调用，Ink 框架自动处理终端差分渲染。
...
| TUI 层 | `@anthropic/ink` (forked Ink 框架, React 19 + Yoga) | 终端渲染与组件树管理。复制自 best-claude-code（146 文件 / ~27K 行），含 Flexbox 布局、虚拟滚动、鼠标交互、Alternate Screen |
